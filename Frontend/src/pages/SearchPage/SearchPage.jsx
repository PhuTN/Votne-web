...
import ProductGridComponent2 from '../../components/ProductPageC/ProductGridComponen/ProductGridComponen2';
...
import { fetchProductsByType, searchProductsByName } from '../../redux/Slicer/productSlice';
...
const { keyword } = useParams(); // Lấy từ khóa từ URL
  ...
  const { productname } = useSelector((state) => state.products); // Lấy dữ liệu từ Redux store
    ...
    const activeProducts2 = productname.filter(product => product.active);
    const filteredProducts = activeProducts2
  .filter(product => {
...
const breadcrumbItems = [
    ...
    { path: `/search/${keyword}`, label: keyword },
  ];
  return (
    <div>
    <ChatBot></ChatBot>
  ...
  <StarFilter selectedRating={selectedRating} setSelectedRating={setSelectedRating} />
          ...
          {/* Main Content */}
          <Content>
            <ProductGridComponent2 title={newKeyword} products={filteredProducts} />
          </Content>
        </PageContainer>
      </div>
    </div>
